...
def process_test(yolo_root, mask_root, out_yolo_root, out_mask_root):
    yolo_root = Path(yolo_root)
    mask_root = Path(mask_root)
    out_yolo_root = Path(out_yolo_root)
    out_mask_root = Path(out_mask_root)

    img_dir = yolo_root / "test" / "images"
    lbl_dir = yolo_root / "test" / "labels"
    msk_dir = mask_root / "test"

    out_img_dir = out_yolo_root / "test" / "images"
    out_lbl_dir = out_yolo_root / "test" / "labels"
    out_msk_dir = out_mask_root / "test"
    ...
    for i, img_path in enumerate(img_paths, 1):
        if i % 200 == 0 or i == total:
            print(f"test: {i}/{total}", flush=True)
        ...
        if not lbl_path.exists():
            raise FileNotFoundError(f"Missing test label: {lbl_path}")
        if not msk_path.exists():
            raise FileNotFoundError(f"Missing test mask: {msk_path}")
        ...
        (out_img_dir / f"{stem}.jpg").write_bytes(b"")  # avoid partial write on crash
...
def main():
    ...
    process_test(YOLO_ROOT, MASK_ROOT, OUT_YOLO_ROOT, OUT_MASK_ROOT)
    print("Done test.")
